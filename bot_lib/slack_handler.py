...
HELP_TEXT = (
    ...
    "  repo·remote·branch 는 컨텍스트(`init`)에서 자동 주입 — 미설정 시 거부\n"
    "  브랜치 명명: who 설정 시 `<type>/<who>/<issue>`, 미설정 시 `<type>/<issue>`\n\n"
    "  예:\n"
    "    run fix CDS-99\n"
    ...
    "    -d: 본문 (그리디, 마지막)\n"
    "    프로젝트는 CDS 고정, assignee 는 API 토큰 소유자로 자동\n"
    "  예:\n"
    ...
    "조회 (평문, read-only):\n"
    "  repo                       - 등록된 repo 목록\n"
    "  remote [<repo>]            - git remote 조회 (인자 없으면 컨텍스트 repo)\n"
    "  remote <name>              - 컨텍스트 remote 를 <name> 으로 변경\n"
    "                                (<name> 이 registry repo 가 아닐 때만 set 동작)\n"
    "  branch [<repo>] [all]      - 최근 브랜치 (기본 10개, `all` 시 최대 50개)\n"
    "  find <pattern> [-r <repo>] - 파일 경로 검색 (case-insensitive, 최대 20개)\n"
    "  who                        - 현재 사용자 이름 + 출처 (context / env / unset)\n"
    "  status                     - 컨텍스트 + repo 요약\n\n"
    "사용자 이름 (who):\n"
    "  who <name>            - 사용자 이름 set (ASCII 영숫자 + `.` `-` `_`)\n"
    "  who clear             - 사용자 이름 해제 (env `BOT_USER` 로 fallback)\n"
    "  → 브랜치 namespace `<type>/<who>/<issue>` 에 사용\n\n"
    "기타:\n"
    "  help / 도움말         - 이 도움말"
)
